...
- **Choix de la langue d'appel (`POST /v1/calls`)** : nouveau champ optionnel `language` à la racine du payload, valeurs `ar` ou `fr`. Permet à un reseller dont le client préfère le français d'avoir un appel en français même depuis un pays maghrébin (ex. `country=MA, language=fr`). Si `language` est omis, la langue locale du pays s'applique automatiquement (`MA`/`DZ`/`TN` → `ar`, `FR` → `fr`) — comportement strictement identique aux versions précédentes, **les resellers existants n'ont rien à changer**. Combinaisons supportées : `MA`/`DZ`/`TN` avec `ar` ou `fr`, `FR` uniquement avec `fr`. La combinaison `country=FR` + `language=ar` est rejetée en `422 language_not_supported_for_country` (cf `docs/errors.md`). La langue effectivement utilisée est ré-écho dans le champ `language` de la réponse `CallOut` et lisible via `GET /v1/calls/{id}` pour audit. Recettes curl dans `docs/examples.md §2.5`.
- **Détail commande structuré sur `POST /v1/calls`** : nouveau champ optionnel `order.items` (tableau, max 50 entrées) qui permet d'envoyer la commande article par article — chaque entrée expose `name` (requis), `quantity` (défaut 1), `variant` et `unit_price` (optionnels). Cohabite avec `order.items_text` existant (résumé libre, max 500 caractères). Les deux champs restent optionnels — un POST sans aucun des deux reste valide. Quand les deux sont fournis, la liste structurée prime au moment de l'appel. Recettes curl complètes dans `docs/examples.md §1` (texte libre) et `§1.bis` (structuré). Le nouveau schéma `OrderItem` est documenté dans `docs/openapi.yaml`.
...
- Documentation interne `doc/whatsapp.md §14.8` : description du comportement de vérification HMAC sur les webhooks entrants alignée sur le code (fail-closed `503` si secret manquant, pas de mode dégradé). Cette section décrivait un mode "warn-puis-poursuit" historique qui n'existe plus depuis M1. Mise en cohérence également de `doc/whatsapp.md §12` (TL;DR) et `doc/call_flow_pipeline.md` (§3.1 + §9.4) qui propageaient la même information obsolète. Aucun changement de comportement runtime.
