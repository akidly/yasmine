...
**18 events `call.*`**. Organisés en 3 rails : DEMANDE (7), TEMPLATE (4 — Phase 3), APPEL (7 — dont service_unavailable). Plus l'event utilitaire `webhook.test` (déclenché par `POST /v1/me/webhooks/test`, payload détaillé en fin de §7). Payloads `data.*` ci-dessous. Envelope standard cf §3.
...
### Utilitaire — `webhook.test`

#### `webhook.test`
Émis **uniquement** quand le reseller appelle `POST /v1/me/webhooks/test` (cf §4 « Tester votre endpoint »). Permet de valider l'intégration côté reseller sans attendre un appel réel.

```json
{
  "test": true,
  "test_id": "01HVZ4QJ9P3K5W7Y8Z2N4F6M8R",
  "request_id": "3f8e9d12-4b5a-4c6d-8e7f-123456789abc"
}
```
- `test` : toujours `true`. Distingue le payload d'un event métier — un handler reseller peut court-circuiter sa logique commerciale sur cette clé.
- `test_id` : ULID unique de la tentative de test. Permet de corréler avec la response synchrone de `POST /v1/me/webhooks/test` (champ `test_id`) et avec la row `webhook_deliveries` (consultable via `GET /v1/me/webhooks/deliveries?event_type=webhook.test`).
- `request_id` : UUID v4 propagé depuis le header `X-Request-ID` de la requête `POST /v1/me/webhooks/test` (ou généré si absent). Pratique pour relier le test côté logs reseller.

L'envelope reste standard (`{id: "evt_...", type: "webhook.test", created_at, livemode: true, data: {...}}`). **Distinct des events `call.*`** : non lié à un appel téléphonique, pas de `call_id` dans `data`. Pas de retry automatique sur `webhook.test` — une seule tentative synchrone (timeout 5 s), le résultat est renvoyé directement par `POST /v1/me/webhooks/test`.
